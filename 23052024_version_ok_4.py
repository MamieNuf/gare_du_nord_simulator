...
def draw():
    ...
    if game_state == TITLE:
        cover_image.draw()
        screen.draw.text("Appuyez sur T pour Thomas, ou B pour Bastien", center=(WIDTH // 2, HEIGHT // 2 + 250), fontsize=30, color="red")
        screen.draw.text("Appuyez sur Enter pour commencer", center=(WIDTH // 2, HEIGHT // 2 + 275), fontsize=30, color="red")
...
def update(dt):
    ...
    for enemy in horizontal_enemies[:]:
        enemy.pos = (enemy.pos[0] - GAME_SPEED * 1.5 * dt, enemy.pos[1])  # Les ennemis horizontaux vont 1.5 fois plus vite
        if enemy.colliderect(hero) and invincibility_time <= 0:
            lives -= 1
            invincibility_time = 2  # 2 secondes d'invincibilité
            if lives == 0:
    ...
    if next_flying_enemy_time <= 0:
        flying_enemy = Actor("rocketrat", anchor=('left', 'center'), pos=(WIDTH, randint(150, 450) ))  # Ajuster la position verticale
        flying_enemies.append(flying_enemy)
        next_flying_enemy_time = randint(FLYING_ENEMY_APPARITION[0],FLYING_ENEMY_APPARITION[1])

    for enemy in flying_enemies[:]:
        enemy.pos = (enemy.pos[0] - GAME_SPEED * 1.5 * dt, enemy.pos[1])  # Les ennemis volants vont 1.5 fois plus vite
        if enemy.colliderect(hero) and invincibility_time <= 0:
            lives -= 1
            invincibility_time = 2  # 2 secondes d'invincibilité
            if lives == 0:
