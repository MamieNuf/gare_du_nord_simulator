...
VERTICAL_ENEMY_APPARITION = (5, 10)  # pigeons
HORIZONTAL_ENEMY_APPARITION = (4, 8)  # rats en voiture
FLYING_ENEMY_APPARITION = (5, 10)  # rats en fusée
...
def draw():
    ...
    if game_state == TITLE:
        screen.draw.text("Evasion Urbaine", center=(WIDTH // 2, HEIGHT // 2 - 100), fontsize=60, color="blue")
        screen.draw.text("Appuyez sur T pour Thomas, ou B pour Bastien", center=(WIDTH // 2, HEIGHT // 2 - 50), fontsize=30, color="white")
        screen.draw.text("Appuyez sur Enter pour commencer", center=(WIDTH // 2, HEIGHT // 2), fontsize=30, color="white")
    elif game_state == GAME_OVER:
        screen.draw.text("Game Over", center=(WIDTH // 2, HEIGHT // 2 - 50), fontsize=60, color="red")
        screen.draw.text(f"Score: {int(score)}", center=(WIDTH // 2, HEIGHT // 2), fontsize=30, color="white")
        screen.draw.text("Appuyez sur Enter pour recommencer", center=(WIDTH // 2, HEIGHT // 2 + 50), fontsize=30, color="white")
        ...
        screen.draw.text(f"Vies: {lives}", topleft=(10, 10), fontsize=30, color="white")
        screen.draw.text(f"Score: {int(score)}", topright=(WIDTH - 10, 10), fontsize=30, color="white")
        screen.draw.text("Appuyez sur P pour mettre en pause", bottomright=(WIDTH - 10, HEIGHT - 10), fontsize=20, color="white")
...
def update(dt):
    ...
    for box in boxes[:]:
        ...
        if box.pos[0] <= -32:
            boxes.remove(box)
            score += 1  # ennemi évité = +1

    # PIGEONS ================================================================================================
    next_vertical_enemy_time -= dt
    if next_vertical_enemy_time <= 0:
    ...
    if next_horizontal_enemy_time <= 0:
        horizontal_enemies.append(Actor("rat", anchor=('left', 'bottom'), pos=(WIDTH, GROUND)))
        next_horizontal_enemy_time = randint(HORIZONTAL_ENEMY_APPARITION[0],HORIZONTAL_ENEMY_APPARITION[1])

    for enemy in horizontal_enemies[:]:
        enemy.pos = (enemy.pos[0] - GAME_SPEED * 2 * dt, enemy.pos[1])  # Les ennemis horizontaux vont 2 fois plus vite
        if enemy.colliderect(hero) and invincibility_time <= 0:
            lives -= 1
            invincibility_time = 2  # 2 secondes d'invincibilité
            if lives == 0:
        ...
        if enemy.pos[0] <= -32:
            ...
            score += 1  # Augmente le score pour chaque ennemi horizontal évité

    # MISE À JOUR DES ENNEMIS VOLANTS ================================================================================================
    next_flying_enemy_time -= dt
    if next_flying_enemy_time <= 0:
        flying_enemies.append(Actor("rocketrat", anchor=('left', 'center'), pos=(WIDTH, randint(100, 350))))
        next_flying_enemy_time = randint(FLYING_ENEMY_APPARITION[0],FLYING_ENEMY_APPARITION[1])

    for enemy in flying_enemies[:]:
        enemy.pos = (enemy.pos[0] - GAME_SPEED * 2 * dt, enemy.pos[1])  # Les ennemis volants vont 2 fois plus vite
        if enemy.colliderect(hero) and invincibility_time <= 0:
            lives -= 1
            invincibility_time = 2  # 2 secondes d'invincibilité
            if lives == 0:
        ...
        if enemy.pos[0] <= -32:
            ...
            score += 1  # Augmente le score pour chaque ennemi volant évité
